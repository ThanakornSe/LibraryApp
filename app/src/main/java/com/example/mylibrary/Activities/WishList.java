...
public class WishList extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        booksRecView = findViewById(R.id.WLRecView);
        booksRecView.setLayoutManager(new LinearLayoutManager(this));

        bookViewModel = new ViewModelProvider.AndroidViewModelFactory(this.getApplication()).create(BookViewModel.class);
        BookViewModel.getBookByPlaylist(LibraryDatabase.WISHLIST).observe(this, new Observer<List<Book>>() {
            @Override
            public void onChanged(List<Book> books) {
                adapter = new BooksRecViewAdapter(WishList.this,BooksRecViewAdapter.WISHLIST,books);
